...
void draw_battery(void)
{
		...
		if(m_a2_info.usb_pin_status == 1)
		{
				...
				{
						if(m_a2_info.usb_charging_status == BATTERY_FULLY_STATE)
						{
				...
				if(m_display_page == DISPLAY_TIME_PAGE)
				{
						if(m_a2_info.usb_charging_status == BATTERY_FULLY_STATE)
						{
								sprintf(buffer_display,"Batt FULL");
								ssd1306_drawRightString(buffer_display,95,-3,2);
						}else{
								sprintf(buffer_display,"Charging");
								ssd1306_drawRightString(buffer_display,95,-3,2);
						}
				}
				...
				}else if(m_a2_info.batter_value > 85){
						battery_level_state = BATTERY_LEVEL_4;
				}else if(m_a2_info.batter_value > 65){
						battery_level_state = BATTERY_LEVEL_3;
				}else if(m_a2_info.batter_value > 45){
						battery_level_state = BATTERY_LEVEL_2;
				}else if(m_a2_info.batter_value > 25){
...
void display_step_count(void)
{
		...
		switch(m_a2_info.current_activity)
		{
			case REST:
				ssd1306_set_cursor(52,2);
				sprintf(buffer_display,"rest");
				ssd1306_putstring(buffer_display);
				break;
			case WALKING:
				ssd1306_set_cursor(52,2);
				sprintf(buffer_display,"walk");
				ssd1306_putstring(buffer_display);
				break;
			case JOGGING:
				ssd1306_set_cursor(48,2);
				sprintf(buffer_display,"jogging");
				ssd1306_putstring(buffer_display);
				break;
			case RUNNING:
				ssd1306_set_cursor(48,2);
				sprintf(buffer_display,"running");
				ssd1306_putstring(buffer_display);
				break;
		}
...
void display_hr_measurementing()
{
		...
		switch(state_count)
		{
				case 0:
						...
						ssd1306_drawRightString(buffer_display,114,13,6);
						state_count++;
						break;
				case 1:
						sprintf(buffer_display,"--");
...
{
		static uint8_t flag_viable_symbol = 0;
		uint16_t max_col_plot;
		char buffer_display[100];
		int str_len;
		...
		if(m_a2_info.broken_max30120_status == true)
		{
				sprintf(buffer_display,"HR sensor is broken.");
				ssd1306_drawRightString(buffer_display,121,20,2);
		}else{
				display_mini_time();
				switch(state_hr)
				{
					case DISPlAY_SHOW_SYMBOL_VALUE:
							...
							//display_hr_measurementing();
							if(flag_viable_symbol == 1)
							{
									sprintf(buffer_display,"--");
									ssd1306_drawRightString(buffer_display,118,13,6);
									flag_viable_symbol = 0;
							}else{
									flag_viable_symbol = 1;
							}
							
						break;
					case DISPlAY_SHOW_HR_MES_VALUE:
							ssd1306_draw_bitmap(20,15,heart_icon,35,35,WHITE);
					...
					case DISPlAY_DO_NOT_SHOW_HR_VALUE:
						...
						break;
					case DISPLAY_GRAPH_HR:
						
						max_col_plot = (m_hr_sample_counter/100)*100;
					 	NRF_LOG_INFO("Max plot %ld", max_col_plot);
						
						break;
				}
				
				if(state_hr != DISPLAY_GRAPH_HR)
				{
...
uint16_t date2days(uint16_t y, uint8_t m, uint8_t d) 
{
    ...
    if (m > 2 && y % 4 == 0)
		...
		res_day = days + 365 * y + (y + 3) / 4 - 1;
		//NRF_LOG_INFO("Y %d, M %d, D %d, Day %d",y,m,d, res_day);
    return res_day;
}
