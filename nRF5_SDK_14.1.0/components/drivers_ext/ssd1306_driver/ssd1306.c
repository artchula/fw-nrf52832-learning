...
static oled_ssd1306_dev_t *p_oled_dev;
...
{   
		ret_code_t ret;
		//uint8_t data_send[] = {0x00, c};
		//ret = nrf_drv_twi_tx(&m_twi_master, _i2caddr, dta_send, 2, false);
		
		p_oled_dev->write( _i2caddr, 0x00, &c, 1 );
...
void ssd1306_begin(oled_ssd1306_dev_t *oled_dev, uint8_t vccstate, uint8_t i2caddr, bool reset)
{
...
void ssd1306_data(uint8_t c)
{
    if (use_i2c) {
        ...
        //ret = nrf_drv_twi_tx(&m_twi_master, _i2caddr, dta_send, 2, false);
				...
				p_oled_dev->write(SSD1306_I2C_ADDRESS, 0x40, &c, 1);
...
void ssd1306_display(void)
{
    ...
    for (uint16_t i=0; i<(SSD1306_LCDWIDTH*SSD1306_LCDHEIGHT/8); i = i+16) {
				...
				uint8_t temp[17];
				temp[0] = 0x40;
				memcpy(&temp[1], &buffer[i], 16);
				//nrf_drv_twi_tx(&m_twi_master, SSD1306_I2C_ADDRESS, &temp[0], 17, false);
			  p_oled_dev->write(SSD1306_I2C_ADDRESS, 0x40, &temp[1], 16);
    }
...
#if 0
void ssd1306_drawFastVLine(int16_t x, int16_t y,
...
void ssd1306_drawFastHLine(int16_t x, int16_t y,
...
***************************************************************************************/
int ssd1306_drawUnicode(unsigned int uniCode, int x, int y, int size)
{
...
#ifdef LOAD_FONT2
		if (size == 2) {
				flash_address = (uint32_t)chrtbl_f16[uniCode] ;
...
#ifdef LOAD_FONT4
		if (size == 4) {
				flash_address = (uint32_t)chrtbl_f32[uniCode];
				width = pgm_read_byte(widtbl_f32+uniCode);
				height = chr_hgt_f32;
				gap = -3;
			
				//uint32_t buf_address = &chrtbl_f32[uniCode];
				
			  //NRF_LOG_INFO("\n uniCode %d , Flash_address ld ", uniCode, flash_address);
		}
#endif
//   if (size == 5) {
...
//   }
		...
		if (size == 6) 
		{
				flash_address = (uint32_t)chrtbl_f64[uniCode];
		...
		if (size == 7) 
		{
				flash_address = (uint32_t)chrtbl_f7s[uniCode];
		...
		for(int i=0; i<height; i++)
		{
				...
				{
						if (textsize == 1) ssd1306_draw_fast_hline(x, pY, width+gap, textbgcolor);
						else ssd1306_fill_rect(x, pY, (width+gap)*textsize, textsize, textbgcolor);
				}   
				...
				{ 
						...
						if(line) {
								...
								{
										pX = x + k*8;
										if(line & 0x80) ssd1306_draw_pixel(pX, pY, textcolor);
										if(line & 0x40) ssd1306_draw_pixel(pX+1, pY, textcolor);
										if(line & 0x20) ssd1306_draw_pixel(pX+2, pY, textcolor);
										if(line & 0x10) ssd1306_draw_pixel(pX+3, pY, textcolor);
										if(line & 0x8) ssd1306_draw_pixel(pX+4, pY, textcolor);
										if(line & 0x4) ssd1306_draw_pixel(pX+5, pY, textcolor);
										if(line & 0x2) ssd1306_draw_pixel(pX+6, pY, textcolor);
										if(line & 0x1) ssd1306_draw_pixel(pX+7, pY, textcolor);
								}
								else{
										pX = x + k*8*textsize;
										if(line & 0x80) ssd1306_fill_rect(pX, pY, textsize, textsize, textcolor);
										if(line & 0x40) ssd1306_fill_rect(pX+textsize, pY, textsize, textsize, textcolor);
										if(line & 0x20) ssd1306_fill_rect(pX+2*textsize, pY, textsize, textsize, textcolor);
										if(line & 0x10) ssd1306_fill_rect(pX+3*textsize, pY, textsize, textsize, textcolor);
										if(line & 0x8) ssd1306_fill_rect(pX+4*textsize, pY, textsize, textsize, textcolor);
										if(line & 0x4) ssd1306_fill_rect(pX+5*textsize, pY, textsize, textsize, textcolor);
										if(line & 0x2) ssd1306_fill_rect(pX+6*textsize, pY, textsize, textsize, textcolor);
										if(line & 0x1) ssd1306_fill_rect(pX+7*textsize, pY, textsize, textsize, textcolor);
								}
						}
				}
				pY+=textsize;
...
/***************************************************************************************
** Function name:           ssd1306_drawNumber unsigned with size
** Descriptions:            ssd1306_drawNumber
***************************************************************************************/
int ssd1306_drawNumber(long long_num,int poX, int poY, int size)
{
    char tmp[10];
    if (long_num < 0) sprintf(tmp, "%li", long_num);
    else sprintf(tmp, "%lu", long_num);
    return ssd1306_drawString(tmp, poX, poY, size);
}
...
***************************************************************************************/
int ssd1306_drawChar(char c, int x, int y, int size)
{
    return ssd1306_drawUnicode(c, x, y, size);
}

/***************************************************************************************
** Function name:           ssd1306_drawString
** Descriptions:            draw string
***************************************************************************************/
int ssd1306_drawString(char *string, int poX, int poY, int size)
{
    int sumX = 0;

    while(*string)
    {
        int xPlus = ssd1306_drawChar(*string, poX, poY, size);
        sumX += xPlus;
        *string++;
...
/***************************************************************************************
** Function name:           ssd1306_drawCentreString
** Descriptions:            draw string across centre
***************************************************************************************/
int ssd1306_drawCentreString(char *string, int dX, int poY, int size)
{
    int sumX = 0;
    int len = 0;
    char *pointer = string;
...
***************************************************************************************/
int ssd1306_drawRightString(char *string, int dX, int poY, int size)
{
    int sumX = 0;
    int len = 0;
    char *pointer = string;
...
** Descriptions:            draw string right justified
***************************************************************************************/
int ssd1306_drawRightStringLen(char *string, int str_len, int dX, int poY, int size)
{
    int sumX = 0;
    int len = 0;
    char *pointer = string;
    ...
    if (poX < 0) poX = 0;
		...
		//NRF_LOG_INFO("dX %d, len %d, poX %d ", dX, len, poX);
		//NRF_LOG_FLUSH();
    for(int i=0;i<str_len;i++)
    {
...
/***************************************************************************************
** Function name:           ssd1306_drawFloat
** Descriptions:            ssd1306_drawFloat
***************************************************************************************/
int ssd1306_drawFloat(float floatNumber, int decimal, int poX, int poY, int size)
{
    ...
    if(floatNumber-0.0f < eep)       // floatNumber < 0
    {
        xPlus = ssd1306_drawChar('-',poX, poY, size);
    ...
    xPlus = ssd1306_drawNumber(temp,poX, poY, size);
